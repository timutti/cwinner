...
fn sound_notes(kind: &SoundKind) -> (Vec<Note>, f32) {
    match kind {
        ...
        SoundKind::Streak => {
            let scale = [
                523.25, 587.33, 659.25, 783.99, 880.0, 1046.5, 1174.7, 1318.5,
            ];
...
mod tests {
    ...
    fn test_sounds_have_distinct_lengths() {
        ...
        assert!(
            mini.len() < fanfare.len(),
            "Mini should be shorter than Fanfare"
        );
        assert!(
            fanfare.len() < streak.len(),
            "Fanfare should be shorter than Streak"
        );
    }
}
